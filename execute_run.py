...
for task in range(ntask):
    ...
    result = subprocess.run(
        ['python', 'run.py', '--output_name', 'main', '--base_model_name', 'deepseek-ai/deepseek-coder-1.3b-base', '--n_perturbation_list', '100', '--n_samples', str(100), '--pct_words_masked', str(0.3), '--span_length', str(2), '--batch_size', str(10), '--skip_baselines', '--task', str(curr_task), '--dataset', 'stability1', '--do_top_p'],
